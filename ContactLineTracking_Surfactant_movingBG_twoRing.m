...
folder='E:\JShi_Durham\A2-Project\EXP\20181117_Spreading_C10E8\C2_5mM_4000fps_30v';
%%% go into the iamge folder
oldFolder=cd(folder);
% provide your video name (the part before numbers) and format
filetitle='C2_5mM_4000fps_30v';
Fileformat ='avi';
...
background_file_end=6853;% the frame where inner film disappears

drying_start_frame=3; % whole drop landed
drying_end_frame=6853; % end of drying

inverse_frame_start=6442;% The frame where there is a very small inner ring
inverse_frame_end=4442;% end frame of your interest


MannualSelect=1; % mannually select 5 points to determine CL

MovingBg=1; % whether to use a moving background; 0: no use
BG_Switchfile=5500; % the frame to switch between start background and moving background
                    % this frame should be at lease pass the spreading stage 
Bg_gap=35; % the gap between calculationg frame and the background frame ahead 



RingMask=1; % decide whether a ring mask is needed, 0 no need
RingMaskNum=1; % the number of ring maskes to be used
frame_RingForm=6600;% frame used to decide the ring size
RingFrameRemove=6400; % the frame above which a mask is added to remove outsider rings
RingWidth=25; % the approximate ring width

frame_RingForm2=4500;% frame used to decide the ring size
RingFrameRemove2=3200; % the frame above which a mask is added to remove outsider rings
RingWidth2=15; % the approximate ring width
skip_frames=49; % take frame+skip_frames+1, no skip: 0
