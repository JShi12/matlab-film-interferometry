...
folder='R:\Jing Shi\A2-Project\EXP\20181018_Spreading_C14E6\D1_5mM_4000fps_30v\D1_video';
%%% go into the iamge folder
oldFolder=cd(folder);
% provide your video name (the part before numbers) and format
filetitle='D1_5mM_4000fps_30v';
Fileformat ='avi';
...
background_file_end=4583;% the frame where inner film disappears
deposit_file=4583; % the frame where inner film disappears

drying_start_frame=2; % whole drop landed
drying_end_frame=4584; % end of drying


inverse_frame_start=4577;% The frame where there is a very small inner ring
inverse_frame_end=4;% end frame of your interest
frame_RingForm=3014;% frame when a ring forms
...
% Remove background 
    ...
    I0= read(frames,inverse_frame_start);
    I0=I0(:,:,1);
    figure (1);
    cla
    imagesc(I0);
...
set(gca, 'XLim', [0.1, 2000],'YLim', [0.1, 2])
xlabel('time / ms');
ylabel('D/D0');
...
fig.InvertHardcopy = 'off';
iptsetpref('ImshowBorder','tight'); % Figures without any borders 
print(fig,'_D-t_t_dry','-dtiff','-r100' );

figure (7)
loglog(time,Dc_dimentionless,'-o')
set(gca, 'XLim', [0.1, 100],'YLim', [1, 2])
xlabel('time / ms');
ylabel('D/D0');
...
fig.InvertHardcopy = 'off';
iptsetpref('ImshowBorder','tight'); % Figures without any borders 
print(fig,'_D-t_t_dry_100ms','-dtiff','-r100' );
