...
folder='D:\JShi_Durham\A2-Project\EXP\20181007_Spreading_C12E8\B1_0.5mM_5000fps_35v';
%%% go into the iamge folder
oldFolder=cd(folder);
% provide your video name (the part before numbers) and format
filetitle='B1_0.5mM_5000fps_35v';
Fileformat ='avi';
...
background_file_end=6738;% the frame where inner film disappears

drying_start_frame=4; % whole drop landed
drying_end_frame=6738; % end of drying

inverse_frame_start=6726;% The frame where there is a very small inner ring
inverse_frame_end=6;% end frame of your interest


MannualSelect=0; % mannually select 6 points to determine CL

MovingBg=0; % whether to use a moving background; 0: no use
Bg_gapNum=2; % the number of background gaps to be used
BG_Switchfile=3300; % the frame to switch between start background and moving background
                    % this frame should be at lease pass the spreading stage (smaller ring close to drying)
Bg_gap=25; % the gap between calculationg frame and the background frame ahead 

BG_Switchfile2=1660; % the frame to switch from Bg_gap tp Bg_gap2(smaller ring close to drying)
Bg_gap2=75; % the gap between calculationg frame and the background frame ahead


RingMask=0; % decide whether a ring mask is needed, 0 no need
RingMaskNum=2; % the number of ring maskes to be used
frame_RingForm=5500;% frame used to decide the ring size(smaller ring close to drying)
RingFrameRemove=5500; % the frame above which a mask is added to remove outsider rings
RingWidth=13; % the approximate ring width

frame_RingForm2=4100;% frame used to decide the ring size(larger ring)
RingFrameRemove2=4200; % the frame above which a mask is added to remove outsider rings
RingWidth2=10; % the approximate ring width
...
% use Matlab build-in edge detection algrithm
    %I=edge(I0,'canny');                 
     I=edge(I0,'sobel'); 
     I =bwareaopen(I, 90);% remove smaller objects
     size_I=size(I);
     figure (3)
%    imshow(BW)
     imagesc(I);
 ...
 %  determine the contour of the deposit ring,get the centroids of deposit ring and the diameter for analysis
    ...
    figure (1)
     ...
     text(1,20,'Deposit Ring','color','r','Fontsize',10);
        ...
        if RingMaskNum==2
        I0= read(frames,frame_RingForm2);
        I0=I0(:,:,1);
        figure (4);
        cla
        imagesc(I0);
...
%    imshow(BW)
 ...
 %  determine the contour of the deposit ring,get the centroids of deposit ring and the diameter for analysis
    ...
    figure (4)
     hold on
     %plot(CL(:,1),CL(:,2),'ro','Markersize',2,'Linewidth',1);
     text(1,20,'Deposit Ring2','color','r','Fontsize',10);
     
        %Fit a circle and extract the centre
        circ_fit=[CL ones(length(CL),1)]\[-(CL(:,1).^2+CL(:,2).^2)];
        xc_depositRing2 = -.5*circ_fit(1);
        yc_depositRing2 = -.5*circ_fit(2);
        R_c_depositRing2  =  sqrt((circ_fit(1)^2+circ_fit(2)^2)/4-circ_fit(3));
        th=0:0.01:2*pi; %%%%Angle
        xy_c=[xc_depositRing2+R_c_depositRing2*cos(th) ; yc_depositRing2+R_c_depositRing2*sin(th)]';
        hold on
        plot(xy_c(:,1),xy_c(:,2),'-r','LineWidth',1)
        plot(xc_depositRing2,yc_depositRing2,'xr','Markersize',10,'Linewidth',1) 
        end
    end  
   % close all;
...
for i=inverse_frame_start:-(skip_frames+1):inverse_frame_end
    ...
    imagesc(I0);
     ...
     ylabel('y /pixel');
     if MannualSelect
        text(1,20,'Evenly select 6 points near contact','color','k','Fontsize',10);
        CL=ginput(6);
     ...
     if MannualSelect==0&& MovingBg==1 && i>BG_Switchfile && i+Bg_gap < background_file_end
     ...
     elseif MannualSelect==0&& MovingBg==1 && Bg_gapNum==2 && i<BG_Switchfile && i> BG_Switchfile2
          % read the image Bg_gap frame ahead as backgroud for substration
        I02=read(frames,i+Bg_gap2);
        I02=I02(:,:,1);
        figure (2);
        cla
        imagesc(I02);
...
% Method 3:  globle threshold method
 ...
 %    I = imbinarize(I0,'adaptive','ForegroundPolarity','bright');
     ...
     if i>=frame_RingForm
     I =bwareaopen(I,200);% remove smaller objects 
     else
     ...
     % Create a circlur mask to remove the deposit ring
      if RingMask==1 && i>=RingFrameRemove
         ...
         I=I.*mask;
      elseif RingMask==1 && RingMaskNum==2 && i<RingFrameRemove && i>=RingFrameRemove2
            [xx,yy] = ndgrid((1:size_I(1))-yc_depositRing2,(1:size_I(2))-xc_depositRing2);
             mask = double((xx.^2 + yy.^2)<(R_c_depositRing2-RingWidth2)^2); % creat a mask which is slightly smaller than the deposit ring
             I=I.*mask;
      end
...
if MannualSelect==0 && MovingBg==1 && i>BG_Switchfile
 ...
 elseif MannualSelect==0 && (MovingBg~=1 || i<=BG_Switchfile)
     [n,m]=find(I); % in matrix row 'n' is coordinates' y and column 'm' is x           
     pklist=[m n];
     indCL=convhull(pklist(:,1),pklist(:,2)); 
     CL=pklist(indCL,:);  
    ...
    % Plot the convex hull of each frame
     ...
     hold on
     %plot(CL(:,1),CL(:,2),'ro','Markersize',2,'Linewidth',1);
     message=sprintf('Frame : %4.0f\nt/t_{dry} : %4.4f',i,round(1000*(i-drying_start_frame)/drying_time_in_frame)/1000);
